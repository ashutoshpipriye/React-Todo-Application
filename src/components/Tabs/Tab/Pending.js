...
import EditTask from '../../Modal/View_task_form';
    ...
    return (
        <div>
            <Table bordered style={{ marginTop: '1rem' }}>
                ...
                <tbody>
                    {props.searchData.map((task, index) => (
                        task.status ? <tr key={index}>
                            ...
                            <td>
                                ...
                                <button className="btn btn-success mr-2" onClick={() => completeTask(task.id)}>{task.status ? "Done" : "Re-open"}</button>
